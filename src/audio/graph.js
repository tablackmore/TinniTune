...
export function buildNature(ctx, kind) {
  ...
  if (kind === 'waves') {
    ...
    const amp = ctx.createGain(); amp.gain.value = 0.64;
    const lfo = ctx.createOscillator(); lfo.type = 'sine'; lfo.frequency.value = 0.05; // ~20s between waves
    const ampDepth = ctx.createGain(); ampDepth.gain.value = 0.08;                      // gentle swell
    const cutoffDepth = ctx.createGain(); cutoffDepth.gain.value = 150;
  ...
  if (kind === 'stream') {
    ...
    const amp = ctx.createGain(); amp.gain.value = 0.64;
    // steady trickle — only a gentle slow drift, no fast tremolo (which read as a wobble)
