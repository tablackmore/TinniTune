 ...
 * @param {{ rate?: number, decaySec?: number, seed?: number, amp?: number }} opts
 ...
 */
export function fillGrains(buf, sampleRate, { rate = 8, decaySec = 0.04, seed = 1, amp = 0.85 } = {}) {
  buf.fill(0);
  ...
  const burstLen = Math.ceil(tau * 6);         // out to ~e^-6 (inaudible)
